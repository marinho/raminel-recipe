...
from raminelrecipe.recipe import Recipe
...
from raminelrecipe import test
from raminelrecipe import manage
...
class TestRecipe(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.buildout_dir = tempfile.mkdtemp('raminelrecipe')
    ...
    def test_make_protocol_scripts(self):
        ...
        self.assert_("application = "
                     "raminelrecipe.wsgi.main('project.development', logfile='')"
                     in contents)
        ...
        self.assert_('project.development' in contents)
        # and a line which set's up the WSGI app
        self.assert_("raminelrecipe.fcgi.main('project.development', logfile='')"
                     in contents)
        self.assert_("class logger(object)" not in contents)
    ...
    def test_create_manage_script_projectegg(self):
        ...
        self.assert_("raminelrecipe.manage.main('spameggs.development')"
                     in open(manage).read())
    ...
    def test_fulfills_django_dependency(self, rmtree, path_exists, 
        ...
        # Test for https://bugs.launchpad.net/raminelrecipe/+bug/397864
        # raminelrecipe should always fulfil the 'Django' requirement.        
    ...
    def test_settings_option(self):
        ...
        self.recipe.create_manage_script([], [])
        manage = os.path.join(self.bin_dir, 'django')
        self.assert_("raminelrecipe.manage.main('project.spameggs')"
                     in open(manage).read())
    ...
    def test_python_option(self):
        ...
        recipe = Recipe({'buildout': {'eggs-directory': self.eggs_dir,
                        ...
                        'django',
                        {'recipe': 'raminelrecipe', 'version': 'trunk',
                         'wsgi': 'true'})
        recipe.make_scripts([], [])
        # This should have created a script in the bin dir
        wsgi_script = os.path.join(self.bin_dir, 'django.wsgi')
...
def setUp(test):
    ...
    cache_dir = os.path.join(tmp, 'raminelrecipe-test-cache')
...
download-cache = %(cache_dir)s
    ...
    zc.buildout.testing.install_develop('raminelrecipe', test)
